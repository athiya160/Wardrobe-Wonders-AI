...
const HeroSection = styled(Box)({
  background: "linear-gradient(135deg, #121418 0%, #1E232B 50%, #2A303C 100%)",
  color: "#FFFFFF",
  ...
  "&::after": {
    ...
    width: "400px",
    height: "100%",
    background: "radial-gradient(circle, rgba(209, 163, 98, 0.15) 0%, rgba(0,0,0,0) 70%)",
    pointerEvents: "none",
  }
});
...
const MEN_CATEGORIES = [
  { key: "Casual", label: "Casual Looks", icon: "👕" },
  { key: "Formal", label: "Suits & Tuxedos", icon: "🤵" },
  { key: "Wedding", label: "Groom Sherwanis", icon: "👑" },
  { key: "Traditional", label: "Festive Kurtas", icon: "🪡" },
  { key: "Party", label: "Party Blazers", icon: "🎉" },
];
...
const AvailableDresses = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${BASE_URL}/products/gender/men`)
      .then((res) => {
        setDresses(Array.isArray(res.data) ? res.data : []);
      })
      .catch((err) => {
        console.error("Failed to load men dresses:", err);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const categoryCounts = useMemo(() => {
    ...
    dresses.forEach((d) => {
      ...
      const match = MEN_CATEGORIES.find(c => c.key.toLowerCase() === cat.toLowerCase());
  ...
  const filteredDresses = useMemo(() => {
    return dresses
      .filter((item) => {
        ...
        // 3. Rental duration
        if (rentalDuration === "7") {
          // 7-day rentals are suitable for wedding, traditional, or formal garments
      ...
      <HeroSection>
        <Container maxWidth="xl">
          ...
          <Box sx={{ maxWidth: "800px" }}>
            ...
            >
              Men&apos;s Designer Collection
            </Typography>
            <Typography variant="body1" sx={{ color: "#B8BEC9", fontSize: "1.05rem", lineHeight: 1.6 }}>
              Explore bespoke Italian wool suits, hand-embroidered royal groom sherwanis, midnight tuxedos, and effortless smart-casual coord sets for any milestone occasion.
            </Typography>
          </Box>
        </Container>
      </HeroSection>
          ...
          <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            >
              <Stack direction="row" spacing={1.5} alignItems="center" flex={1}>
                ...
                <TextField
                  size="small"
                  placeholder="Search suits, sherwanis, kurtas..."
                  value={searchQuery}
            ...
            {loading ? (
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary" sx={{ maxWidth: 450, mx: "auto", mb: 3 }}>
                  We couldn&apos;t find any men&apos;s outfits matching your selected filters. Try broadening your price range or clearing some category tags.
                </Typography>
                <Button
                  variant="outlined"
